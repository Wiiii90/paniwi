...
export function selectEffectiveGoalsForScorers(goals: GoalRecord[]): GoalRecord[] {
  ...
  for (const [key, aggregateGoals] of aggregatesByScorer) {
    ...
    effectiveGoals.push(...(sumGoalPoints(aggregateGoals) > sumGoalPoints(detailedGoals) ? aggregateGoals : detailedGoals));
  }
